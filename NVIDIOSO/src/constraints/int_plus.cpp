...
IntPlus::consistency () 
{
   	  ...
   	  else if ( _scope_size == 2 ) 
   	  {
   	  	...
   	  	if ( !is_variable_at( 2 ) ) 
   	  	{
   	  		...
   	  		if ( !_var_x->is_singleton() ) 
   	  		{
   	  			...
   	  			int min_x = _arguments[ 0 ] - _var_y->min ();
                
   	  			int max_x = _arguments[ 0 ] - _var_y->max ();
 ...
 max_z >= _var_z->min () )
	 	...
	 	if ( !_var_y->is_singleton() ) 
	 	{
	 		...
	 		int min_y = _var_z->min () - _var_x->min ();
            
	 		int max_y = _var_z->max () - _var_x->max ();
 ...
 max_y >= _var_y->min () )
	 	...
	 	if ( !_var_x->is_singleton() ) 
	 	{
	 		...
	 		int min_x = _var_z->min () - _var_y->min ();
            
	 		int max_x = _var_z->max () - _var_y->max ();
